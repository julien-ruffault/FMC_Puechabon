...
output_path               <-  paste0(mainDir,'/test_FMC_Puechabon_campbell.csv')
...
DATA_day$PLC_Leaf = aggregate(DATA$PLC_Leaf,by=list(yday(DATA$Time),year(DATA$Time)),max)$x
...
quartz()
plot(DATA_day$Date,DATA_day$Psi_min,type='l',col=2,ylim=c(-6,0),main=year,)
lines(DATA_day$Date,DATA_day$Psi_base,type='l',col=1)
par(new=T)
plot(DATA_day$Date,DATA_day$PLC_Leaf,type='l',col=2, ylab="",main=year,ylim=c(0,100), yaxt="n")

quartz()
plot(DATA$Psi_AllSoil,type='l',col=2,main=year)
lines(DATA$Psi_SSym,type='l',col=3,main=year,)
lines(DATA$Psi_LSym,type='l',col=4,main=year,)
