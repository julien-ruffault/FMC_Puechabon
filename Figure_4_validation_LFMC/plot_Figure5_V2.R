...
mtext('A',adj=0,col=1,side=3,font=2,cex=0.8)
 ...
 par(new=T, plt=c(0.1,0.4,0.06,0.41),xpd=F)
 plot(1:10,type='n',ylim=c(0,0.2),xlim=c(0.5,3.5),axes=F,xlab="",ylab="")
 abline(v=1:3,h=seq(0,0.2,0.05),col=gray(.8,1),lty='dotted')
 boxplot(DIFRESNDVI$var~DIFRESNDVI$year, ylim=c(0, 0.2),xlab="",ylab="",axes=F,add=T,lwd=0.5,pch=16,cex=0.7,outcol=gray(0.2,0.5),staplewex=0.1,whisklty=1)
 box(lwd=.5)
 axis(2,las=2,cex.axis=cexx,lwd=0,lwd.ticks=0.5,tck=-0.02,mgp = c(2,0.5,0))
 axis(1,at=1:3,lab=2016:2018,cex.axis=cexx,lwd=0,lwd.ticks=0,tck=-0.02,mgp = c(2,0.2,0))
mtext(side=2,'NDVI anomaly',line=2,cex=cexx)
 
mtext('B',adj=0,col=1,side=3,font=2,cex=0.8)


# 
# par(new=T,plt=c(0.85,0.92,0.08,0.42))
# library(grid)
# grid.raster(pp, x=.5, y=.5, width=.4)
# 

 par(new=T,plt=c(0.45,0.92,0.06,0.41))
 plot(0:1,0:1,type='n',xaxs='i',yaxs='i',axes=F,xlab='',ylab='')
 rasterImage(pp, 0,0,1,1,xaxs='i',yaxs='i')
 mtext("C",side=3, font=2,cex=0.8,adj=0)
