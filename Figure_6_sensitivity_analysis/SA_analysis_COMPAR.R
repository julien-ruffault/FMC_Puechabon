...
percentV = 20/100
N <- 30 # number for initial sampling / about 30000-100000 for full models
...
compOptionsForEvapo = c("Fast"),
thresholdMortality=99)
Out_dir <- file.path(mainDirOutpout2)
dir.create(Out_dir,showWarnings=F,recursive=T)
write.csv(PARAMS,paste0(Out_dir,'/PARAMS_.csv'),row.names=F)
foreach(i=1:nrow(PARAMS),.packages=c('lubridate','insol')) %dopar% {
  print(i)
...
for (i in 1:nrow(PARAMS))
{
  ...
  Z3[i]  = io$yearly_nbDayLFMC_57
}


# Z1 :  sans defol
# Y1 : avec defol
plot(Y1,Z1,ylab = 'FMCmin (sans defol)',xlab = 'FMCmin (avec defol)',pch=16,col=grey(0.2,0.5))
abline(0,1,col='red')
