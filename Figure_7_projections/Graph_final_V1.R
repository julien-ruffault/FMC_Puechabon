# Authors : julien ruffault (julien.ruff@gmail.com) & Francois Pimont (francois.pimont@inrae.fr)
# Date : 04/04/2022
# Data formatting and computing derived statistics for FMC projections from climate models 
...
#plot.new()
par(mfrow=c(1,2),mar=c(1,2.5,1,1.5),oma=c(2,.5,0,0))
...
plot(t[io],y85[io,ivar],ylim=c(0,70),xlim=c(2005,2099),type='n',xlab=NA,ylab=NA,xaxs="i",yaxs="i",axes=F)
# model
polygon(c(rev(t[io]),t[io]),c(rev(y85[io,ivar]),y85pm[io,ivar])-offset,col=modcol,border=F)
polygon(c(rev(t[io]),t[io]),c(rev(y45[io,ivar]),y45mm[io,ivar])-offset,col=modcol,border=F)
lines(t[io],y85pm[io,ivar],col=climcol,cex=2)
lines(t[io],y45mm[io,ivar],col=climcol,cex=2)
# scenario
polygon(c(rev(t),t),c(rev(y45[,ivar]),y85[,ivar])-offset,col=scecol,border=F) 
lines(t[io],y85[io,ivar]-offset,col='black',cex=2)
lines(t[io],y45[io,ivar]-offset,col='black',cex=2,lty=2)
# climat
y85pmc[io,ivar]=myfilt(y85pmc[io,ivar],filter_30,iopt)
y45mmc[io,ivar]=myfilt(y45mmc[io,ivar],filter_30,iopt)
...
axis(1,lwd.ticks=.3,lwd=0,cex.axis=0.72,tck=-0.02,mgp=c(2,.3,0),at=seq(2010,2100,10),labels=F)
...
plot(t[io],y85[io,ivar],ylim=c(0,100),type='n',xlab=NA,ylab=NA,xaxs="i",yaxs="i",axes=F)
# model
polygon(c(rev(t[io]),t[io]),c(rev(y85[io,ivar]),y85pm[io,ivar])-offset,col=modcol,border=F)
polygon(c(rev(t[io]),t[io]),c(rev(y45[io,ivar]),y45mm[io,ivar])-offset,col=modcol,border=F)
lines(t[io],y85pm[io,ivar],col=climcol,cex=2)
...
y85pmc[io,ivar]=myfilt(y85pmc[io,ivar],filter_30,iopt)
y45mmc[io,ivar]=myfilt(y45mmc[io,ivar],filter_30,iopt)
polygon(c(rev(t[io]),t[io]),c(rev(y85pm[io,ivar]),y85pmc[io,ivar])-offset,col=climcol,border=F)
polygon(c(rev(t[io]),t[io]),c(rev(y45mm[io,ivar]),y45mmc[io,ivar])-offset,col=climcol,border=F)
#lines(t,y85pmc[,ivar]-offset,col='black',cex=2);lines(t,y45mmc[,ivar]-offset,col='black',cex=2)
...
axis(2,las=2,lwd.ticks=.5,lwd=0,cex.axis=0.7,tck=-0.03,mgp=c(2,.6,0))
box(lwd=.5)
mtext(side=1,cex=0.8,'Year',line=1.2)
mtext(side=2,cex=0.8,'PLC (%)',line=1.8)
mtext(side=3,cex=0.8,'B',adj=0,font=2)


# 
legend(x=2007,y=99, bty="n",cex=0.68,
       legend=c('Decadal mean under RCP4.5',"Decadal mean under RCP8.5","Scenario uncertainty","Model uncertainty","Internal uncertainty"),
       lwd=c(1.2,1.2,0,0,0),
       lty=c(2,1,NA,NA,NA),
       pt.cex=1.2,
